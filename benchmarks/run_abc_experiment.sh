...
# Guard settings apply to candidate-on only in affinity-guard mode. The
# overload escape valve was removed in 0.2.x after experiments showed only
# false-positive escapes (see docs/design.md), so the mode now isolates
# miss-unbind plus optional cache-discounted reservations.
...
deploy_proxy_group() {
  ...
  log "${group}: deploying proxy variant=${proxy_variant} kv_aware=${kv_aware} decoder_count=${decoder_count} miss_unbind_threshold=${miss_unbind_threshold} cache_discount_alpha=${cache_discount_alpha}"
...
verify_proxy_group() {
  ...
  local expected_miss_unbind_threshold=0 expected_cache_discount_alpha=0
  ...
  local actual_miss_unbind_threshold actual_cache_discount_alpha
  ...
  read -r actual_variant actual_kv_aware actual_active_token_weight actual_miss_unbind_threshold actual_cache_discount_alpha < <(kubectl -n "${NAMESPACE}" exec "${pod_name}" -- python -c \
    'import os; print(os.environ.get("PROXY_VARIANT", ""), os.environ.get("KV_AWARE_ROUTING", ""), os.environ.get("PREFILL_ACTIVE_TOKEN_WEIGHT", ""), os.environ.get("AFFINITY_MISS_UNBIND_THRESHOLD", "0"), os.environ.get("AFFINITY_CACHE_DISCOUNT_ALPHA", "0"))')
  ...
  if [[ "${actual_sha}" != "${expected_sha}" || "${actual_variant}" != "${expected_variant}" || "${actual_kv_aware}" != "${expected_kv_aware}" || "${actual_active_token_weight}" != "${expected_active_token_weight}" || "${actual_miss_unbind_threshold}" != "${expected_miss_unbind_threshold}" || "${actual_cache_discount_alpha}" != "${expected_cache_discount_alpha}" ]]; then
    echo "${group}: proxy identity mismatch expected=${expected_variant}/${expected_kv_aware}/weight-${expected_active_token_weight}/guard-${expected_miss_unbind_threshold}-${expected_cache_discount_alpha}/${expected_sha} actual=${actual_variant}/${actual_kv_aware}/weight-${actual_active_token_weight}/guard-${actual_miss_unbind_threshold}-${actual_cache_discount_alpha}/${actual_sha}" >&2
    return 1
  fi

  "${PYTHON_BIN}" -c \
    'import json,sys; print(json.dumps(dict(zip(("group","comparison_mode","expected_variant","actual_variant","expected_kv_aware","actual_kv_aware","expected_active_token_weight","actual_active_token_weight","expected_affinity_miss_unbind_threshold","actual_affinity_miss_unbind_threshold","expected_affinity_cache_discount_alpha","actual_affinity_cache_discount_alpha","expected_source_sha256","actual_source_sha256","pod_name","pod_uid","image_id"), sys.argv[1:])), separators=(",", ":")))' \
